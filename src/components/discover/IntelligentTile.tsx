    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
          {isTrending && (
            <span className="chip-trending flex items-center gap-1 text-xs font-medium px-2 py-0.5 sm:px-2.5 sm:py-1">
              <TrendingUp className="w-3 h-3" />
              Trending
            </span>
          )}
          {isHot && (
            <span className="flex items-center gap-1 text-xs font-medium px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-gradient-primary text-white">
              <Flame className="w-3 h-3" />
              Hot
            </span>
          )}
          {dealText && (
            <span className="flex items-center gap-1 text-xs font-medium px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-accent text-accent-foreground">
              {dealText}
            </span>
          )}
        </div>
        ...
        {/* Rating badge */}
        {rating && (
          <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-black/50 backdrop-blur-sm">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4">
          ...
          {/* Title */}
          <h3 className="font-display text-base sm:text-xl font-bold text-white mb-1 line-clamp-2 group-hover:text-primary-glow transition-colors">
            {title}
          </h3>
      ...
      {/* AI Context footer */}
      {aiContext && (
        <div className="p-2.5 sm:p-3 border-t border-border/50 bg-muted/30 mt-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground min-w-0">
              <ContextIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-ai-pulse shrink-0" />
              <span className="truncate">{aiContext.reason}</span>
            </div>
